...
repack_sonic_initrd_for_tftp_installer() {
    ...
    cp "$PLATFORM_ASPEED/aspeed-platform-services/scripts/sonic-uboot-env-init.sh" "$d/sbin/sonic-uboot-env-init.sh"
    chmod 755 "$d/sbin/sonic-uboot-env-init.sh"
    cp "$PLATFORM_ASPEED/aspeed-platform-services/scripts/sonic-machine-conf-init.sh" "$d/sbin/sonic-machine-conf-init.sh"
    chmod 755 "$d/sbin/sonic-machine-conf-init.sh"
    cp "$PLATFORM_ASPEED/aspeed-platform-services/scripts/sonic-fw-env-config.sh" "$d/sbin/sonic-fw-env-config.sh"
